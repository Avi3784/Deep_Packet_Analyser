...
bool DPIEngine::initialize() {
    ...
    // Create and load ML traffic classifier model.
    ml_classifier_ = std::make_unique<MLTrafficClassifier>();
    if (!config_.ml_model_file.empty()) {
        if (ml_classifier_->loadModel(config_.ml_model_file)) {
            std::cout << "[DPIEngine] ML model loaded: " << config_.ml_model_file << "\n";
        } else {
            std::cerr << "[DPIEngine] Warning: failed to load ML model from '"
                      << config_.ml_model_file
...
PacketJob DPIEngine::createPacketJob(const PacketAnalyzer::RawPacket& raw,
    ...
    // Safely calculate protocol offsets.
    if (job.data.size() > 14) {
        uint8_t ip_ihl = job.data[14] & 0x0F;
        size_t ip_header_len = static_cast<size_t>(ip_ihl) * 4;

        // IPv4 minimum header size is 20 bytes.
        if (ip_header_len < 20) {
            ip_header_len = 20;
        }

        job.transport_offset = 14 + ip_header_len;
        if (job.transport_offset >= job.data.size()) {
            return job;
        }

        if (parsed.has_tcp) {
            // Need at least 13 bytes into TCP header to read data-offset field.
            if (job.transport_offset + 12 >= job.data.size()) {
                return job;
            }

            uint8_t tcp_data_offset = (job.data[job.transport_offset + 12] >> 4) & 0x0F;
            size_t tcp_header_len = static_cast<size_t>(tcp_data_offset) * 4;
            if (tcp_header_len < 20) {
                tcp_header_len = 20;
            }
            job.payload_offset = job.transport_offset + tcp_header_len;
        } else if (parsed.has_udp) {
            // UDP header is 8 bytes; ensure complete header is present.
            if (job.transport_offset + 8 > job.data.size()) {
                return job;
            }
