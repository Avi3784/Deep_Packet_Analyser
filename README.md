...
| Domain | `*.youtube.com` | Domain/SNI matches (supports wildcard suffix patterns) |
| Port | `443` | All traffic to destination port 443 |

### Rules File Format (`--rules <file>`)

The engine supports loading rules from a file with these sections:
...
You can use the included demo file `blocking_rules_demo.txt`.
...
**Multi-threaded Version (DPIEngine + ML classification):**
```bash
...
**Use custom ML model file:**
```bash
./dpi_engine test_dpi.pcap output.pcap --ml-model models/traffic_dt_model.txt
```
...
**With rules file:**
```bash
./dpi_engine test_dpi.pcap output.pcap --rules blocking_rules_demo.txt
```
...
From parent `DPI` folder (wrapper script):
```powershell
powershell -ExecutionPolicy Bypass -File .\run_dpi.ps1 \
    -InputPcap test_dpi.pcap \
    -OutputPcap out.pcap \
...
Classification is done per connection (five-tuple flow). Each flow is labeled as one of:

- Generic protocol labels: `HTTP`, `HTTPS`, `DNS`, `TLS`, `QUIC`
- App/service labels: `Google`, `YouTube`, `Facebook`, `Instagram`, `Twitter/X`, `Netflix`, `Amazon`, `Microsoft`, `Apple`, `WhatsApp`, `Telegram`, `TikTok`, `Spotify`, `Zoom`, `Discord`, `GitHub`, `Cloudflare`
- Fallback: `Unknown`

Classification is label-only (no confidence score output).
...
### ML Traffic Classification Notes

- Classification is label-only (no confidence score output).
- Default model path: `models/traffic_dt_model.txt`.
- To train your own decision-tree model, prepare a CSV and run:

```bash
python3 train_ml_classifier.py --csv your_training_data.csv --out models/traffic_dt_model.txt
```
...
### How to Confirm Blocking is Actually Active

If blocking is working, you should see all of these in terminal output:

1. Rule load lines (for `--rules`):
    - `[RuleManager] Rules loaded from: ...`
    - plus individual `Blocked ...` lines
2. Runtime block lines:
    - `[FPx] BLOCKED packet: ...`
3. Final stats show non-zero blocked traffic:
    - `Dropped/Blocked: > 0`
    - `Blocked Apps/Domains/Ports` counters match your rules
