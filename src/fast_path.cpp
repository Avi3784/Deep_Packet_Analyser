...
FastPathProcessor::FastPathProcessor(int fp_id,
                         ...
                         const MLTrafficClassifier* ml_classifier,
                                     PacketOutputCallback output_callback)
    : fp_id_(fp_id),
      input_queue_(10000),
      conn_tracker_(fp_id),
      rule_manager_(rule_manager),
    ml_classifier_(ml_classifier),
...
void FastPathProcessor::inspectPayload(PacketJob& job, Connection* conn) {
    ...
    bool has_sni = tryExtractSNI(job, conn);
    bool has_http_host = tryExtractHTTPHost(job, conn);

    if (ml_classifier_ && ml_classifier_->isLoaded()) {
        auto features = MLTrafficClassifier::buildFeatures(job, *conn, has_sni, has_http_host);
        AppType predicted = ml_classifier_->predict(features);
        if (predicted != AppType::UNKNOWN) {
            conn_tracker_.classifyConnection(conn, predicted, conn->sni);
    ...
    // Fallback if model is unavailable or predicts Unknown.
    if (has_sni || has_http_host) {
        AppType app = sniToAppType(conn->sni);
        conn_tracker_.classifyConnection(conn, app, conn->sni);
        if (app != AppType::UNKNOWN && app != AppType::HTTP && app != AppType::HTTPS) {
            classification_hits_++;
        }
        return;
    }
...
FPManager::FPManager(int num_fps,
    ...
    for (int i = 0; i < num_fps; i++) {
        auto fp = std::make_unique<FastPathProcessor>(i, rule_manager, ml_classifier, output_callback);
